<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=24&duration=2500&pause=1000&color=000000&background=FFFFFF&center=true&width=800&height=80&lines=👋+Hi+there!+I'm+Bishnu+Prasad;Full+Stack+Developer+|+ML+Engineer" alt="Typing Animation" />

</div>

<div align="center">

<a href="https://www.linkedin.com/in/bishnuprasad-tripathy/">
  <img src="https://img.shields.io/badge/LINKEDIN-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
</a>
<a href="https://github.com/bishnu-prasad">
  <img src="https://img.shields.io/badge/GITHUB-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub">
</a>

</div>

---

## 👋 Welcome to My GitHub!

Hi, I’m **Bishnu Prasad** — a Full-Stack Developer and Machine Learning Engineer passionate about building scalable web applications, intelligent systems, and real-time data solutions.

I work primarily with Python, FastAPI, React, Next.js, TypeScript, PostgreSQL, MongoDB, and Machine Learning. I enjoy turning ideas into practical, production-focused applications and continuously improving my skills through real-world projects.

---

## 🛠️ Technical Skills

### Languages
`Python` `JavaScript` `TypeScript` `SQL` `HTML` `CSS`

### Frontend
`React.js` `Next.js` `React Native` `Tailwind CSS` `Shadcn UI` `Framer Motion` `GSAP`

### Backend
`FastAPI` `Django` `Node.js` `REST APIs` `Pydantic` `SQLAlchemy` `Alembic` `Uvicorn`

### Databases
`PostgreSQL` `MySQL` `MongoDB` `SQLite` `TimescaleDB` `ClickHouse`

### Machine Learning & Data
`NumPy` `Pandas` `Scikit-learn` `OpenCV`

### Authentication & Security
`JWT` `HttpOnly Cookies` `Access & Refresh Tokens` `Argon2` `CORS`

### Tools & Infrastructure
`Git` `GitHub` `Docker` `Redis` `MQTT` `Mosquitto` `Streamlit` `Plotly`

---

## ⚙️ What I Build

- Full-Stack Web Applications
- RESTful APIs
- Machine Learning Applications
- Real-Time Monitoring Systems
- Multi-Tenant SaaS Platforms
- Authentication & Authorization Systems
- Data Processing & Analytics Systems
- Database-Driven Applications
- Enterprise Backend Systems

---

## 🚀 Featured Projects

| Project | Description | Tech Stack |
|---|---|---|
| 🩺 **Predixa** | Full-stack platform for multi-disease prediction using Machine Learning | Python, FastAPI, React, Next.js, MongoDB, Scikit-learn, OpenCV |
| 📊 **FreeRADIUS AI Monitoring** | Real-time monitoring and analysis of FreeRADIUS authentication logs | Python, Watchdog, Regex, Streamlit, Plotly, Redis, PostgreSQL |
| 🛡️ **Women's Safety Companion** | Mobile safety application with location tracking and emergency features | React Native, Expo, FastAPI, MongoDB, Google Maps API, Twilio |
| 💓 **CardioPredict** | Machine Learning model for heart disease prediction | Python, Pandas, NumPy, Scikit-learn, FastAPI |

---

## 💡 Currently Working On

- 🔧 Building scalable Full-Stack applications
- 🐍 Developing production-ready Python & FastAPI backends
- ⚛️ Building modern applications with React & Next.js
- 🤖 Developing and integrating Machine Learning models
- 🗄️ Working with PostgreSQL, MongoDB and SQL databases
- 📡 Building real-time monitoring and telemetry systems
- 🏗️ Learning and implementing enterprise software architecture
- 🔐 Implementing secure authentication and authorization systems
- 🚀 Improving deployment, DevOps and cloud development skills
- 📚 Continuously learning new technologies and best practices

---

## 🎯 Current Focus

My current focus is on becoming a stronger Full-Stack Python Developer with expertise in:

**Python** → **FastAPI** → **PostgreSQL** → **SQLAlchemy** → **React/Next.js** → **TypeScript** → **Docker** → **Cloud & DevOps**

I'm also exploring Machine Learning integration, real-time systems, scalable backend architecture, and enterprise application development.

---

## 📊 GitHub Statistics

<details>
  <summary><b>📈 GitHub Stats & Analytics</b></summary>
  <br/>
  <div align="center">
    <a href="https://github.com/bishnu-prasad">
      <img alt="Bishnu's GitHub Stats" src="https://github-readme-stats.vercel.app/api?username=bishnu-prasad&show_icons=true&count_private=true&theme=default&hide_border=false&bg_color=ffffff&title_color=000000&text_color=000000&icon_color=000000" height="192px" />
    </a>
    <a href="https://github.com/bishnu-prasad">
      <img alt="Bishnu's Top Languages" src="https://github-readme-stats.vercel.app/api/top-langs/?username=bishnu-prasad&langs_count=12&layout=compact&theme=default&hide_border=false&bg_color=ffffff&title_color=000000&text_color=000000" height="192px" />
    </a>
  </div>
</details>

## 🔥 Contribution Activity

<div align="center">
  <a href="https://github.com/bishnu-prasad">
    <img alt="Bishnu's GitHub Activity" src="https://github-readme-activity-graph.vercel.app/graph?username=bishnu-prasad&bg_color=ffffff&color=000000&line=000000&point=000000&area=false&hide_border=false&custom_title=Contribution%20Activity" width="97%" />
  </a>
  <br/><br/>
  <img src="https://raw.githubusercontent.com/bishnu-prasad/bishnu-prasad/output/github-contribution-grid-snake.svg" alt="Bishnu's GitHub Contribution Grid Snake" width="97%" />
</div>

---

## 📬 Get In Touch

| Platform | Link |
|---|---|
| 📧 **Email** | [bishnuprasadtripathy.work@gmail.com](mailto:bishnuprasadtripathy.work@gmail.com) |
| 💼 **LinkedIn** | [linkedin.com/in/bishnuprasad-tripathy](https://www.linkedin.com/in/bishnuprasad-tripathy/) |
| 📝 **LeetCode** | [leetcode.com](https://leetcode.com/) |
| 🐙 **GitHub** | [github.com/bishnu-prasad](https://github.com/bishnu-prasad) |

---

<div align="center">

### ⭐ If you find my work interesting, consider giving a star! ⭐

**Made with ❤️ by Bishnu Prasad**

![Profile Views](https://komarev.com/ghpvc/?username=bishnu-prasad&label=PROFILE+VIEWS&style=flat-square&color=black)

Last Updated: August 2026 | Actively Maintained ✨

</div>
